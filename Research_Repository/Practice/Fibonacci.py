...
while int(p) < 3:
    p = input(
        "Please enter a number corresponding to the position in the sequence (should be at least 3:) "
    )
...
while q <= 0:
    q = int(
        input(
            "Please enter number of terms to generate (should be a positive integer): "
        )
    )
